...
- 第三方安装说明（Issue，非官方）：`<在此替换为 Issue 链接>`
...
4. **模型配置**  
   ...
   配置完成后，返回与 TopoClaw 的会话，即可在下图（图 4_1）所示位置选择模型。  
...
- **跨用户执行**  
  ...
  - 群组：群组由多用户、多助手组成，可由用户或 TopoClaw 发起。支持三种编排方式，可在群组主页配置：  
    ...
    3) 工作流编排模式：点击群组会话页右上角进入工作流编排（图 5_1）；可由用户手动编排，或由 TopoClaw 代为编排。完成后，群内助手将按工作流协同执行。

    ![图 5_1：工作流编排](./imgs/Image_5_1.png)
