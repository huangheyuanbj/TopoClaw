...
class TopoMobileRelayHub:
    ...
    async def connect(self, imei_id: str, websocket: WebSocket) -> bool:
        ...
        async with self._lock:
            ...
            owner_hard_stale = bool(owner_age_s >= self.OWNER_HARD_STALE_SECONDS)
            # Strong guard: once a known owner is online, do not allow unknown newcomer takeover
            # unless owner is hard-stale. This stops frequent unknown->known oscillation.
            if old and owner_instance and not owner_hard_stale:
                logger.warning(
                    "TopoMobile node connection denied by owner guard: imei_id=%s owner_conn_id=%s owner_remote=%s owner_instance_id=%s owner_age_s=%.1f lease_ttl_s=%.1f hard_stale_s=%.1f newcomer_conn_id=%s newcomer_remote=%s",
                    imei_id,
